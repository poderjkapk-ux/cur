...
# --- СТВОРЮЄМО ПАПКУ ДЛЯ APK ЯКЩО ЇЇ НЕМАЄ ---
APK_DIR = "static/apks"
os.makedirs(APK_DIR, exist_ok=True)

# --- КОНФІГУРАЦІЯ APK (ВЕРСІЇ ДОДАТКІВ) ---
APK_CONFIG_FILE = "apk_config.json"
DEFAULT_APK_CONFIG = {
    "courier": {"versionCode": 0, "versionName": "1.0.0"},
    "partner": {"versionCode": 0, "versionName": "1.0.0"}
}

def load_apk_config():
    if not os.path.exists(APK_CONFIG_FILE):
        with open(APK_CONFIG_FILE, 'w') as f:
            json.dump(DEFAULT_APK_CONFIG, f)
        return DEFAULT_APK_CONFIG
    with open(APK_CONFIG_FILE, 'r') as f:
        return json.load(f)

def save_apk_config(config):
    with open(APK_CONFIG_FILE, 'w') as f:
...
def get_delivery_admin_html(couriers, partners, pwa_config, apk_config, tz_string="Europe/Kiev", message="", cash_balance=0.0, cash_transactions=None, non_cash_deposits=None, active_jobs=None, announcements=None):
    ...
    <style>
        ...
        .pwa-settings input, .apk-settings input {{ background: rgba(0,0,0,0.2); padding: 8px; border: 1px solid #475569; border-radius: 6px; color: white; width: 100%; margin-bottom: 10px; box-sizing: border-box;}}
        .pwa-settings label, .apk-settings label {{ font-size: 0.8rem; color: #94a3b8; margin-bottom: 2px; display: block; }}
    </style>
    </head>
    <body>
        <div style="max-width: 1400px; margin: 0 auto; padding: 20px;">
            ...
            <div class="panel" style="margin-bottom: 20px; border: 1px dashed #3b82f6;">
                ...
                <div class="grid" style="grid-template-columns: 1fr 1fr;">
                    ...
                    <div style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 10px;" class="apk-settings">
                        <h3 style="margin-top:0; color: #facc15;">Додаток Кур'єра</h3>
                        <p style="font-size:0.85rem; color:#94a3b8; margin-top:-10px;">
                            Поточна версія: <b>{apk_config['courier']['versionName']}</b> (Code: {apk_config['courier']['versionCode']})
                        </p>
                        <form action="/admin/delivery/upload_apk" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="app_type" value="courier">
                            
                            <label>Новий Version Code (ціле число, напр. 2):</label>
                            <input type="number" name="version_code" required min="1">
                            
                            <label>Новий Version Name (текст, напр. 1.0.1):</label>
                            <input type="text" name="version_name" required placeholder="Наприклад: 1.0.1">
                            
                            <label>APK Файл:</label>
                            <input type="file" name="file" accept=".apk" required style="padding: 5px;">
                            
                            <button type="submit" class="btn success" style="width: 100%; margin-top:10px;"><i class="fa-solid fa-upload"></i> Завантажити оновлення</button>
                        </form>
                        <div style="margin-top: 15px; font-size: 0.85rem;">
                            <a href="/download-apk/courier" target="_blank" style="color:#3b82f6;"><i class="fa-solid fa-download"></i> Завантажити поточний APK</a>
                        </div>
                    </div>

                    <div style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 10px;" class="apk-settings">
                        <h3 style="margin-top:0; color: #facc15;">Додаток Ресторану (Партнера)</h3>
                        <p style="font-size:0.85rem; color:#94a3b8; margin-top:-10px;">
                            Поточна версія: <b>{apk_config['partner']['versionName']}</b> (Code: {apk_config['partner']['versionCode']})
                        </p>
                        <form action="/admin/delivery/upload_apk" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="app_type" value="partner">
                        ...
                        <div style="margin-top: 15px; font-size: 0.85rem;">
                            <a href="/download-apk/partner" target="_blank" style="color:#3b82f6;"><i class="fa-solid fa-download"></i> Завантажити поточний APK</a>
                        </div>
                    </div>
            ...
            <div class="panel" style="margin-top: 20px;">
                ...
                <form method="post" action="/admin/delivery/pwa_save" class="pwa-settings grid" style="grid-template-columns: 1fr 1fr;">
                    ...
                    <div style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 10px;">
                        <h3 style="margin-top:0">Partner PWA</h3>
                        <label>Назва додатка</label>
...
# ==============================================================================
#                 НОВІ ЕНДПОІНТИ ДЛЯ ANDROID APK UPDATER
# ==============================================================================

@router.post("/admin/delivery/upload_apk")
async def upload_apk(
    app_type: str = Form(...), # 'courier' або 'partner'
    ...
    if app_type not in ["courier", "partner"]:
        return RedirectResponse("/admin/delivery?message=Невідомий тип додатка", status_code=302)
        
    if not file.filename.endswith('.apk'):
        return RedirectResponse("/admin/delivery?message=Файл має бути формату .apk", status_code=302)

    # Зберігаємо файл у папку static/apks/
    file_path = os.path.join(APK_DIR, f"{app_type}.apk")
    ...
    save_apk_config(config)
    
    return RedirectResponse(f"/admin/delivery?message=Оновлення успішно завантажено! Версія: {version_name}", status_code=302)


@router.get("/api/check-update/{app_type}")
async def check_app_update(app_type: str, request: Request):
    if app_type not in ["courier", "partner"]:
        raise HTTPException(status_code=404, detail="App type not found")
        
    config = load_apk_config()
    # Якщо ключа немає, беремо безпечне значення за замовчуванням
    app_data = config.get(app_type) or {"versionCode": 0, "versionName": "1.0.0"}
    
    host = request.headers.get("x-forwarded-host", request.headers.get("host", "restify.site"))
    download_url = f"https://{host}/download-apk/{app_type}"
    
    return JSONResponse({
        "success": True,
        "app": app_type,
        "latest_version_code": app_data.get("versionCode", 0),
        "latest_version_name": app_data.get("versionName", "1.0.0"),
